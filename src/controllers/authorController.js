...
const editAuthor = async (req, res, next) => {
    ...
    try {
        ...
        // Verificar si req.author está definido
        if (!req.author || !req.author.id) {
            return next(new HttpError("Author not authenticated", 401));
        }
        ...
        if(emailExist && (emailExist._id != req.author.id)) {
            return next(new HttpError("Email already exist.", 422 ))
            
        }
        //Compare current password to db password
        const validateAuthorPassword = await bcrypt.compare(currentPassword, author.password);
...
module.exports = {registerAuthor , loginAuthor , getAuthor , changeAvatar, editAuthor, getAuthors}